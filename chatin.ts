...
import { ChatCompletionFunctions, ChatCompletionRequestMessage, CreateCompletionResponseUsage, OpenAIApi} from 'openai';
 ...
 */
export function ChatParameter(data: { description : string, type: 'string' | 'object', required? : boolean}) {
    return function (target: any, propertyKey: string, parameterIndex: number) : void {
        Reflect.defineMetadata(`chat_function_${parameterIndex}`, { ...data, name: propertyKey }, target, propertyKey);
    }
}
...
export class ChatFunctionsController {
     ...
     */
    constructor(public openai : OpenAIApi, options? : Partial<ChatFunctionsControllerOptions> ) {
        this.options = {
    ...
    registerFunctions(plugin : any, options? : Partial<RegisterFunctionsOptions>) { 
        ...
        for (const toEnable of opts.enable) {
            this.enabledFunctions.push(opts.namespace ? `${opts.namespace}.${toEnable}` : toEnable)
        }

        (Object.getOwnPropertyNames(Object.getPrototypeOf(plugin))
        .filter((name => Reflect.hasMetadata("chat_function", plugin, name))))
        .forEach((name) => {
            const metadata = Reflect.getMetadata("chat_function", plugin, name) as { name: string, description: string };
            const parameterData = []
            const parameterRequired = []
            const parameterIndex = new Map<string, number>();
            let i = 0;
            const namespaced = opts.namespace ? `${opts.namespace}.${metadata.name}` : metadata.name;
            while (Reflect.hasMetadata(`chat_function_${i}`, plugin, name)) {
                const metadata = Reflect.getMetadata(`chat_function_${i}`, plugin, name) as { name: string, description: string, type: 'string' | 'object', required? : boolean};
                parameterData.push(metadata);
                if(metadata.required) { parameterRequired.push(metadata.name); }
        ...
        return Array.from(this.registeredFunctionsMap, ([name, data]) => {
            return {
                ...
                parameters: {
                    type: 'object',
                    properties: {
                        ...data.parameterData.reduce((acc, cur) => { acc[cur.name] = { type: cur.type, description: cur.description }; return acc; }, {} as any)
                    }
                },
    ...
    async handleFunctionCall(name : string, args : string) : Promise<string> {
        ...
        if (!funcData) {
            ...
            if (!funcData.parameterRequired.every((r) => Object.getOwnPropertyNames(parsedArgs).includes(r))) {
                throw new Error(`Missing required parameters (needed "${funcData.parameterRequired.join(', ')}", got ${Object.getOwnPropertyNames(parsedArgs).join(', ')})`);
            }
     ...
     */
    async getFunctionMessageFromMessage(message : ChatCompletionRequestMessage, options?: Partial<GetFunctionMessageOptions>) : Promise<ChatCompletionRequestMessage> {
        if (!message.function_call) {
            throw new Error('Message does not have a function call');
        }

        return this.getFunctionMessage(message.function_call.name!, message.function_call.arguments!, options);
    }
     ...
     * @returns The function message.
     */
    async getFunctionMessage(name : string, args : string, options?: Partial<GetFunctionMessageOptions>) : Promise<ChatCompletionRequestMessage> {
        ...
        try {
            ...
            while (encode(truncatedResult).length > opts.requestMaxTokens) {
                if (truncatedResult.length > 4 * opts.requestMaxTokens) { 
                    truncatedResult = truncatedResult.substring(0,  4 * opts.requestMaxTokens)
                }
                else {
                    truncatedResult = truncatedResult.substring(0, truncatedResult.length - 4)
                }
            }
     ...
     */
    async getCompletionWithMessage(message: ChatCompletionRequestMessage, options?: Partial<GetCompletionOptions>) {
    ...
    async getNextCompletion(options? : Partial<GetCompletionOptions>) {
        ...
        const opt : GetCompletionOptions = { model : this.options.model, 
                      ...options };
